# M1.10 · CLI polish (`cmd/voidslice`)
...
Three small follow-up gaps left by M1.6: ANSI color output (stubbed but unimplemented), multi-file argument support, and a `--strict` flag. Each sub-task is independent and can be done in order.
