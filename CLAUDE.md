...
- **Commit messages:** default to subject line only — no body. Reach for a body only when the user has explicitly asked for one or the *why* is impossible to infer from the diff. Match the bar of `git log --oneline -10`. The log is for skimming, not reading.
